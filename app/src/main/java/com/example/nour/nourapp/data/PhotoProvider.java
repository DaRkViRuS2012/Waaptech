...
public class PhotoProvider
{
	...
	{
		try {
			File cacheDir = WaapTechApp.getAppContext().getExternalCacheDir();
			if (cacheDir == null){//cache dir not available
				cacheDir = WaapTechApp.getAppContext().getCacheDir();
			}
			...
			ImageLoaderConfiguration.Builder builder = new Builder(WaapTechApp.getAppContext());
...
//			builder.memoryCache(new UsingFreqLimitedMemoryCache(2000000));
	...
	private void checkLastCacheClearTimestamp()
	{
		try {
			...
			long timenow = WaapTechApp.getTimestampNow();
			// compare times
