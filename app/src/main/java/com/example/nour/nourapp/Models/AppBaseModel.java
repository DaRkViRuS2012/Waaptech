package com.example.nour.nourapp.Models;
...
public abstract class AppBaseModel {
    ...
    public JSONObject getJsonObject(){
        try {
            WaapTechApp.getSharedGsonParser().toJson(this);
            return new JSONObject(WaapTechApp.getSharedGsonParser().toJson(this));
        }catch (Exception e){}
    ...
    public static <T> ArrayList<T> getArrayFromJsonSting(String jsonString, Type objectType) {
        try {
            ArrayList<T> object = WaapTechApp.getSharedGsonParser().fromJson(jsonString, objectType);
            return object;
        }catch (Exception e){
            e.printStackTrace();
        }
