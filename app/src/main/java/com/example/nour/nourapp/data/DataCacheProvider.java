...
import com.example.nour.nourapp.Models.AppBaseModel;
...
public class DataCacheProvider {
	...
	private DataCacheProvider() {
		try {
			// initialize
			prefData = WaapTechApp.appContext.getSharedPreferences("app_data", Context.MODE_PRIVATE);
    ...
    public void storeObjectWithKey(String key, Object arrayGroups) {
        try {
            if (arrayGroups != null) {
                String strJson = WaapTechApp.getSharedGsonParser().toJson(arrayGroups);
    ...
    public <T> T getStoredObjectWithKey(String key, Type objectType) {
        try {
            ...
            T object = WaapTechApp.getSharedGsonParser().fromJson(str, objectType);
            return object;
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    public <T> ArrayList<T> getStoredArrayWithKey(String key, Type objectType) {
        try {
            String str = prefData.getString(key, null);
            ArrayList<T> object = WaapTechApp.getSharedGsonParser().fromJson(str, objectType);
            return object;
        }catch (Exception e){
            e.printStackTrace();
        }
